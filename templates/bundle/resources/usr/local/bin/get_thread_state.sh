...
function compare {
    ...
    do
        ((total++))
        found=$(awk -v h="${hash}" -v n="${name}" -v s="${state}" \
            '$1==h && $2==n && $3==s {print}' "${2}")
        [[ -n "${found}" ]] && ((match++))
    done < "${1}"
...
function main {
    ...
    if (( "${comparison[1]}" >= 90 && "${comparison[2]}" >= 90 ))
    then
        local msg="Comparison is above 90% threshold"
        [[ "${comparison[1]}" -eq 100 && "${comparison[2]}" -eq 100 ]] && msg="All threads are matching"
        echo "Lifecycle monitor: ${msg}."
        exit 2
    ...
    then
        ...
        echo "Lifecycle monitor: Comparison is below threshold."
        exit 0
    fi
}
