...
function check_if_tag_exists {
	...
	then
		...
		then
			lc_log DEBUG "Add ${hotfix_zip_file} to ${IGNORE_ZIP_FILES_CACHE_FILE}"
...
function check_ignore_via_argument {
	...
	then
		...
		lc_log DEBUG "The file ${hotfix_zip_file} is not on the command line defined ignore list"
...
function check_ignore_via_file {
	...
	then
		...
		lc_log DEBUG "The file on '${file_url}' is not in the ${ignore_zip_file}."
...
function check_usage {
	...
	LIFERAY_COMMON_DOWNLOAD_MAX_TIME="3600"
...
function get_hotfixes_url {
	...
	echo "https://hotfixes.liferay.com/${partial_url}"
}
...
function get_hotfix_zip_list_file {
	...
	then
		...
		lc_log DEBUG "Generating the zip list file ${zip_list_file} from hotfixes.liferay.com."

		lc_curl "$(get_hotfixes_url "${release_version}")/" | \
			grep --only-matching --extended-regexp "href=\"liferay-(dxp|hotfix)[^\"]*\.zip\"" | \
...
function process_argument_version {
	...
	do
		lc_curl "$(get_hotfixes_url "${release_version}")/" &> /dev/null
...
function process_zip_list_file {
	...
	do
		...
		local file_url="$(get_hotfixes_url "${release_version}")/${hotfix_zip_file}"
