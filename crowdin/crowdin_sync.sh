...
function check_usage {
	...
	_LIFERAY_PORTAL_REPOSITORY_NAME="liferay-portal"
...
function close_existing_pull_request {
	...
	do
		local existing_pull_request=$(gh pr list \
			--jq ".[0]" \
			--json "number" \
			--repo "${repository_owner}/${_LIFERAY_PORTAL_REPOSITORY_NAME}" \
			--search "head:crowdin-translations" \
			--state "open")

		if [ -z "${existing_pull_request}" ]
		then
			lc_log INFO "No existing pull request found in ${repository_owner}/${_LIFERAY_PORTAL_REPOSITORY_NAME}."
		...
		gh pr close "${pull_request_number}" --repo "${repository_owner}/${_LIFERAY_PORTAL_REPOSITORY_NAME}"
	done
}
...
function create_pull_request {
	...
	then
		git remote add liferay-release "git@github.com:liferay-release/${_LIFERAY_PORTAL_REPOSITORY_NAME}.git"
	fi
	...
	if [ "${?}" -ne 0 ]
	then
		lc_log ERROR "Unable to push branch ${_TEMP_BRANCH} to liferay-release/${_LIFERAY_PORTAL_REPOSITORY_NAME}."
	...
	gh pr create \
		...
		--repo "liferay-release/${_LIFERAY_PORTAL_REPOSITORY_NAME}" \
...
function set_up_branch {
	...
	cp "${_CROWDIN_DIR}/crowdin.yml" "${_PROJECTS_DIR}/${_LIFERAY_PORTAL_REPOSITORY_NAME}"
}
...
function update_portal_repository {
	...
	then
		git remote add upstream "git@github.com:liferay/${_LIFERAY_PORTAL_REPOSITORY_NAME}.git"
	fi
