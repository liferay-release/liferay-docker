...
function build_bundle_image {
	...
	then
		bundle_url="releases-cdn.liferay.com/dxp/nightly/$( \
			curl \
				--fail \
				--location \
				--show-error \
				--silent \
				"https://releases.liferay.com/dxp/nightly/.lfrrelease-tomcat-bundle")"
	fi

	if [ -z "${bundle_url}" ]
	then
		bundle_url="releases-cdn.liferay.com/dxp/${version}/$( \
			curl \
				--fail \
				--location \
				--show-error \
				--silent \
				"https://releases-cdn.liferay.com/dxp/${version}/.lfrrelease-tomcat-bundle")"
	fi
...
function build_caddy_image {
	...
	LIFERAY_DOCKER_IMAGE_PLATFORMS="${LIFERAY_DOCKER_IMAGE_PLATFORMS}" LIFERAY_DOCKER_LATEST=true LIFERAY_DOCKER_REPOSITORY="${LIFERAY_DOCKER_REPOSITORY}" time ./build_caddy_image.sh "${BUILD_ALL_IMAGES_PUSH}" | tee --append "${LIFERAY_DOCKER_LOGS_DIR}/caddy.log"
...
function get_tag_from_image {
	...
	then
		...
		then
			...
			version=$( \
				docker image inspect --format '{{index .Config.Labels }}' "${image_name}:latest" | \
				grep --only-matching "${filter}" | \
				sed --expression "s/.*://g")
		fi
