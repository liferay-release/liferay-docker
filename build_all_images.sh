...
function build_bundle_image {
	...
	if [ -z "${bundle_url}" ]
	then
		bundle_url="releases-cdn.liferay.com/dxp/${version}/"$(curl --location --show-error --silent --fail "https://releases-cdn.liferay.com/dxp/${version}/.lfrrelease-tomcat-bundle")
	fi
...
function get_latest_docker_hub_version {
	...
	local version=$(curl --header --silent "Authorization: Bearer $token" "https://registry-1.docker.io/v2/liferay/${1}/manifests/latest" | grep --only-matching '\\"org.label-schema.version\\":\\"[0-9]\.[0-9]\.[0-9]*\\"' | head -1 | sed 's/\\"//g' | sed 's:.*\:::')
...
function get_latest_docker_hub_zabbix_server_version {
	...
	local version=$(curl --header --silent "Authorization: Bearer $token" "https://registry-1.docker.io/v2/${image_tag}/manifests/${tag}" | grep --only-matching "\\\\\"${label_name}\\\\\":\\\\\"[0-9]*\.[0-9]*\.[0-9]*\\\\\"" | head -1 | sed 's/\\"//g' | sed 's:.*\:::')
...
function get_latest_docker_hub_zulu_version {
	...
	local version=$(curl --header --silent "Authorization: Bearer $token" "https://registry-1.docker.io/v2/liferay/${1}/manifests/latest" | grep --only-matching "\\\\\"org.label-schema.zulu${2}_${3}_version\\\\\":\\\\\"[0-9]*\.[0-9]*\.[0-9]*\\\\\"" | head -1 | sed 's/\\"//g' | sed 's:.*\:::')
