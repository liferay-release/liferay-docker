...
function trigger_ci_test_suite {
	...
	then
		if [ -z "${JENKINS_CI_TRIGGER_API_KEY}" ]
		then
			lc_log ERROR "Set the environment variable \"JENKINS_CI_TRIGGER_API_KEY\"."
		...
		do
			local http_code=$( \
				curl \
					...
					--data-urlencode "TEST_PORTAL_REPOSITORY_NAME=${LIFERAY_PORTAL_REPOSITORY_NAME}" \
					...
					--fail \
					--header "X-Api-Key: ${JENKINS_CI_TRIGGER_API_KEY}" \
					--max-time 10 \
					--output /dev/null \
					--request "POST" \
					--retry 3 \
