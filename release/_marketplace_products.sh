...
function test_marketplace_products_compatibility {
	...
	for product_name in $(printf "%s\n" "${!LIFERAY_MARKETPLACE_PRODUCTS[@]}" | sort --ignore-case)
	do
		lc_log INFO "Downloading product ${product_name}."

		_download_product_by_external_reference_code "${LIFERAY_MARKETPLACE_PRODUCTS[${product_name}]}" "${product_name}"
		...
		lc_log INFO "Deploying product zip file ${product_name}.zip to ${_BUNDLES_DIR}/deploy."

		_deploy_product_zip_file "${_BUILD_DIR}/marketplace/${product_name}.zip"
	...
	for product_name in $(printf "%s\n" "${!LIFERAY_MARKETPLACE_PRODUCTS[@]}" | sort --ignore-case)
	do
		_test_product "${LIFERAY_MARKETPLACE_PRODUCTS[${product_name}]}" "${product_name}"
...
function _test_product {
	...
	lc_log INFO "Testing the compatibility of product ${product_name} with ${_PRODUCT_VERSION} release."

	if [ ! -f "${_BUILD_DIR}/marketplace/${product_name}.zip" ]
	then
		lc_log ERROR "Unable to test product ${product_name} because the product zip file ${product_name}.zip was not downloaded."
	...
	if [ -z "${modules_info}" ]
	then
		lc_log ERROR "Unable to test product ${product_name}."
	...
	then
		...
		echo "${modules_info}" | while IFS= read -r module_info
		do
			local module_name=$(echo "${module_info}" | awk -F "|" '{print $4}' | sed 's/ (.*)//' | xargs)

			lc_log ERROR "Module ${module_name} is not compatible with release ${_PRODUCT_VERSION}."

			local module_id=$(echo "${module_info}" | awk -F "|" '{print $1}')
...
function _update_product_supported_versions {
	...
	then
		...
		then
			lc_log ERROR "Unable to update the list of supported versions for product ${product_name}. HTTP status: ${http_response}."
