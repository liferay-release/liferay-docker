...
function _is_supported_product_version {
	...
	then
		...
		then
			release_date=$(_get_general_availability_date "dxp" "2024.q1.1")
			years=3
		elif is_lts_release "${product_version}"
		then
			release_date=$(_get_general_availability_date "dxp" "${product_group_version}.0-lts")
			years=3
		else
			release_date=$(_get_general_availability_date "dxp" "${product_group_version}.0")
			years=1
		fi
	elif [ "${product_version}" == "7.4.13-u92" ]
	then
		release_date=$(_get_general_availability_date "dxp" "7.4.13-u92")
		years=4
	fi
	...
	local end_of_premium_support_date=$(date --date "${release_date} +${years} year -1 day" +%Y-%m-%d)
...
function _process_product_version {
	...
	then
		lc_log INFO "Skipping ${product_name} ${product_version} because the general availability date is invalid."
...
function _tag_supported_product_versions {
	...
	while read -r json_file
	do
		while read -r product_version_json_url
		do
