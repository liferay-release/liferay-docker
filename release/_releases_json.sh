...
function _get_supported_product_group_versions {
	local supported_product_group_versions="$( \
		ls \
			"${_PROMOTION_DIR}"| \
			grep --extended-regexp --only-matching "(7+\.[2-4]+(\.q[1-4])?|20[0-9][0-9]\.q[1-4])" | \
			sort --unique)"

	local last_product_version="$( \
		echo "${supported_product_group_versions}" | \
			grep ".q" | \
			tail --lines=1)"

	local year=$(get_release_year "${last_product_version}")
	local quarter=$(get_release_quarter "${last_product_version}")
