...
function add_ckeditor_license {
	...
	if (is_early_product_version_than "2025.q2.0" || is_equals_or_later_product_version_than "2026.q1.0-lts")
	then
		lc_log INFO "The product version is outside the 2025.q2 to 2025.q4 release window."
...
function get_java_specification_version {
	...
	if echo "${JAVA_HOME}" | grep --extended-regexp "openjdk-17|zulu-17" &> /dev/null
	then
		echo "17"
	fi
