...
function add_property {
	...
	sed -i "/${search_key}/a\	\\${new_key}=${new_value}" "${file}"
}
...
function reference_new_releases {
	...
	local previous_quarterly_release_branch_name="$(grep "portal.latest.bundle.version" "build.properties" | tail -1 | cut -d '[' -f 2 | cut -d ']' -f 1)"
...
function replace_property {
	...
	sed -i "s/${search_key}/${new_key}=${new_value}/" "${file}"
}
